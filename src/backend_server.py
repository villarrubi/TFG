...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--port", type=int, default=_env_int("BACKEND_PORT", 8766))
    parser.add_argument(
        "--mode", choices=sorted(VALID_MODES), default=os.getenv("BACKEND_MODE", MODO_COMBINADO)
    )
    parser.add_argument("--threshold", type=float, default=_env_float("BACKEND_THRESHOLD", 45.0))
    parser.add_argument("--heur-weight", type=int, default=_env_int("BACKEND_HEUR_WEIGHT", 60))
    parser.add_argument("--neural-weight", type=int, default=_env_int("BACKEND_NEURAL_WEIGHT", 40))
